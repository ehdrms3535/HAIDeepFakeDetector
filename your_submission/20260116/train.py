...
def build_model_for_train(unfreeze_last_k: int = 1, train_classifier: bool = True) -> SwinForImageClassification:
    """
    Swin-Base 기준 stage는 0~3 (총 4개).
    - unfreeze_last_k=1 => stage3만 학습 (추천)
    - unfreeze_last_k=2 => stage2+stage3 학습
    - unfreeze_last_k=0 => classifier만 학습

    이 구현은 HuggingFace Swin 파라미터 네이밍(prefix)로 stage를 강제 지정해서,
    의도치 않게 84M/86M처럼 거의 전체가 풀리는 사고를 막는다.
    """
    ...
    # 3) stage 선택 (Swin encoder stages: 0,1,2,3)
    #    HuggingFace 파라미터 prefix: "swin.encoder.layers.{stage}."
    #    (버전에 따라 "swin.encoder.layers.{stage}." 가 표준)
    ...
    stages_to_unfreeze = list(range(4 - unfreeze_last_k, 4))  # 예: k=1 -> [3], k=2 -> [2,3]
    prefixes = [f"swin.encoder.layers.{s}." for s in stages_to_unfreeze]

    # ✅ 혹시 다른 네이밍(구버전/변형) 대비: 대안 prefix도 함께 시도
    ...
    # 안전장치: 너무 많이 풀렸으면 경고
    # (Swin-Base total ~86.7M. 60M 넘으면 거의 전체 언프리즈 가능성 큼)
    if trainable > 60_000_000:
        print("⚠️ WARNING: trainable params가 너무 큽니다. stage 언프리즈가 과도하게 적용됐을 수 있어요.")
        print("   => unfreeze_last_k를 1로 낮추거나, prefix 매칭 로그를 확인하세요.")
...
def train_main(args):
    ...
    if not Path("train.csv").exists() or not Path("val.csv").exists() or args.remake_csv:
        make_train_val_csv(TRAIN_ROOT, "train.csv", "val.csv", test_size=args.val_ratio,use_ratio=0.5)
...
if __name__ == "__main__":
    ...
    p_train.add_argument("--unfreeze_last_k", type=int, default=1) # stage 단위 얘가 높으면 더 많이 푼다 ex) 2면 2,3 stage 푼다
