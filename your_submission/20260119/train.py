...
class RandomJPEGCompression:
    """
    PIL 이미지를 저품질 JPEG로 한번 저장했다가 다시 로드해서
    압축 아티팩트를 주입
    """
    def __init__(self, p=0.7, qmin=25, qmax=60):
        self.p = p
        self.qmin = qmin
        self.qmax = qmax
...
train_transform = transforms.Compose([
    ...
    RandomJPEGCompression(p=0.7, qmin=25, qmax=60),  # ✅ 이 줄 추가
...
# =========================
# ✅ 배치 프레임 합쳐서 1번에 forward (GPU 활용률 해결 핵심)
# =========================
def forward_logits_batched(model, xs_list: List[torch.Tensor]) -> List[torch.Tensor]:
    """
    xs_list: List([T,3,H,W]) length=B
    return:  List([T]) length=B
    """
    lens = [x.shape[0] for x in xs_list]
    x_cat = torch.cat(xs_list, dim=0).to(DEVICE, non_blocking=True)  # [sumT,3,H,W]

    outputs = model(pixel_values=x_cat)
    logits = outputs.logits.squeeze(-1)  # [sumT]

    splits = torch.split(logits, lens, dim=0)  # tuple of [T]
    return list(splits)
...
def eval_auc(model, loader, loss_fn, agg_method: str, topk_ratio: float, save_pred_path: Optional[str] = None):
    ...
    for xs_list, y, names in loader:
        y = y.to(DEVICE)  # [B,1] 또는 [B]
        logits_list = forward_logits_batched(model, xs_list)

        for logits_t, yi, nm in zip(logits_list, y, names):
            yi = yi.view(1)
    ...
    if save_pred_path is not None:
        pd.DataFrame({"filename": fns, "pred": ps, "label": ys}).to_csv(save_pred_path, index=False)
        print("saved preds:", save_pred_path)

    return float(np.mean(losses)), float(auc)


def train_one_epoch(model, loader, optimizer, scheduler, scaler, loss_fn, agg_method: str, topk_ratio: float):
    model.train()
    losses = []
    ...
    for xs_list, y, _names in tqdm(loader, desc="Train", leave=False):
        ...
        with torch.amp.autocast("cuda", enabled=(DEVICE == "cuda")):
            ...
            batch_loss = 0.0
            for logits_t, yi in zip(logits_list, y):
                yi = yi.view(1)
                target_t = yi.expand_as(logits_t)
                loss = loss_fn(logits_t, target_t)
...
def train_main(args):
    ...
    for ep in range(start_epoch, args.epochs + 1):
        tr_loss = train_one_epoch(model, train_loader, optimizer, scheduler, scaler, loss_fn, args.agg, args.topk_ratio)
...
def predict_one_file(model, file_path: Path, num_frames: int, face_crop: bool, agg: str, topk_ratio: float) -> float:
    ...
    logits_t = outputs.logits.squeeze(-1)
    if logits_t.dim() == 0:
...
def infer_main(args):
    ...
    files = sorted([p for p in root.rglob("*") if p.is_file() and p.suffix.lower() in (IMAGE_EXTS | VIDEO_EXTS)])
...
if __name__ == "__main__":
    ...
    p_train.add_argument("--use_ratio", type=float, default=0.5)
